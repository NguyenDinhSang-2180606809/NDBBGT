...
test_img_dir = os.path.join(dataset_path, "test", "images")
test_images = os.listdir(test_img_dir)
random_indices = np.random.randint(0, len(test_images), 5)  # Chọn 5 ảnh ngẫu nhiên

plt.figure(figsize=(40, 50))  # Tăng kích thước: rộng 20, cao 25 cho 5 hàng
for i, idx in enumerate(random_indices):
    img_path = os.path.join(test_img_dir, test_images[idx])
    img = cv2.imread(img_path)
    img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    ...
    # Cột trái: Ảnh gốc với nhãn CNN
    plt.subplot(5, 2, 2 * i + 1)  # 5 hàng, 2 cột, vị trí lẻ
    plt.imshow(img_rgb)
    plt.title(f"CNN: {cnn_label}", fontsize=14)  # Tăng cỡ chữ tiêu đề
    plt.axis('off')

    # Cột phải: Ảnh YOLOv8 với hộp và nhãn YOLO
    plt.subplot(5, 2, 2 * i + 2)  # 5 hàng, 2 cột, vị trí chẵn
    plt.imshow(yolo_img)
    plt.title("YOLOv8", fontsize=14)  # Tăng cỡ chữ tiêu đề
    plt.axis('off')
